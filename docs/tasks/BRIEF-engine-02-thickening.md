...
Poprzednik: `BRIEF-engine-01-vertical-slice.md` (krok 1 — DOMKNIĘTY: szkielet,
zaślepka `simulate()`, `golden_preset()`, 13 testów).
...
Zamienić zaślepkę w prawdziwy silnik: `simulate(golden_preset())` ma zwrócić
`SimResult` z `reached_orbit=True`, prawdziwą telemetrią lotu i werdyktem
policzonym z elementów keplerowskich. Po kroku 2 złoty scenariusz demo działa
na PRAWDZIWEJ fizyce: lepszy preset → orbita, gorszy parametr → brak orbity,
z policzonych liczb, nie z zaślepki.
...
2. **Faza 1 (wznoszenie) realna — jeśli nie domknięta w kroku 1.**
   Całkowanie: ciąg + grawitacja g(h) + opór ρ(h), zadany profil kąta (start
   pionowy → pochylanie wg `launch_angle_deg`; NIE gravity turn). `solve_ivp`.
   Telemetria z prawdziwych próbek. (Jeśli krok 1 to już zrobił — pomiń.)
...
3. **Faza 2 (wstawienie) + sklejenie segmentów.** Opór znikomy (próg D/mg<ε jako
   ...
   punktu 1. PO TYM PUNKCIE: `simulate(golden_preset())` ma dać `reached_orbit=True`.
...
## Fallback uproszczony — wciąż JAWNY (przypomnienie z kroku 1)
...
niedziałający realistyczny o 4 nad ranem. Krok 1 mógł już ten fallback zostawić —
jeśli tak, krok 2 go nie usuwa, tylko dokłada gałąź realistyczną obok.

---

## Kryteria akceptacji

- `simulate(golden_preset())` → `reached_orbit=True`, z prawdziwych ε, e, r_p
  (nie z zaślepki). Perygeum > R_EARTH + MIN_PERIAPSIS_ALTITUDE, e < MAX_ECCENTRICITY_LEO.
- Test negatywny z PRAWDZIWEJ fizyki: gorszy parametr (np. skrócony burn_time S1
  ...
  demo" (zgłoś do PROGRESS.md — frontend tego użyje w Akcie 2 demo).
...
2. Dopiero potem, jeśli to realne straty: wydłuż `burn_time` S2 lub S3 o ~10–15 %
   (więcej paliwa = większe Δv) i ZGŁOŚ zmianę presetu do PROGRESS.md i człowieka
   — preset to wspólna kotwica, jego zmianę się sygnalizuje.
...
- Zgłoś: preset osiąga orbitę na prawdziwej fizyce + zarekomendowana wartość
  „suwaka demo" (parametr i próg, przy którym werdykt spada).
